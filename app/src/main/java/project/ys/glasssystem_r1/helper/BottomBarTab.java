package project.ys.glasssystem_r1.helper;
...
//    }
    ...
    public BottomBarTab(Context context, @DrawableRes int iconUnselected, @DrawableRes int iconSelected, CharSequence title) {
        this(context, null, 0, iconUnselected, iconSelected, title);
    }

    public BottomBarTab(Context context, @DrawableRes int iconSelected, CharSequence title) {
...
//    }
    ...
    public BottomBarTab(Context context, AttributeSet attrs, int defStyleAttr, @DrawableRes int iconUnselected, @DrawableRes int iconSelected, CharSequence title) {
        super(context, attrs, defStyleAttr);
        init(context, iconUnselected, iconSelected, title);
    }
    ...
    private void init(Context context, int iconUnselected, int iconSelected, CharSequence title) {
        mContext = context;
        TypedArray typedArray = context.obtainStyledAttributes(new int[]{R.attr.selectableItemBackgroundBorderless});
        Drawable drawable = typedArray.getDrawable(0);
        setBackgroundDrawable(drawable);
        ...
        int size = (int) TypedValue.applyDimension(TypedValue.COMPLEX_UNIT_DIP, 27, getResources().getDisplayMetrics());
        LinearLayout.LayoutParams params = new LinearLayout.LayoutParams(size, size);
        mIcon.setImageResource(mIconUnselected);
        mIcon.setLayoutParams(params);
        mIcon.setColorFilter(ContextCompat.getColor(context, R.color.tab_unselect));
        lLContainer.addView(mIcon);
    ...
    public void setSelected(boolean selected) {
        ...
        if (selected) {
            ...
            mIcon.setImageDrawable(ContextCompat.getDrawable(mContext, mIconSelected));
            mTvTitle.setTextColor(ContextCompat.getColor(mContext, R.color.colorPrimary));
        } else {
            mIcon.setColorFilter(ContextCompat.getColor(mContext, R.color.tab_unselect));
            mIcon.setImageDrawable(ContextCompat.getDrawable(mContext, mIconUnselected));
